...
func (l *amqpEventListener) Listen(eventNames ...string) (<-chan msgqueue.Event, <-chan error, error) {
	...
	go func() {
		for msg := range msgs {
			...
			switch eventName {
			case "flight.booked.failure":
				event = new(domainEvents.BookingValidatedFailureEvent)
			case "flight.booked.success":
				event = new(domainEvents.BookingValidatedSuccessfullyEvent)
			case "process.payment.failed":
				event = new(domainEvents.ProcessPaymentFailureEvent)
			case "process.payment.success":
				event = new(domainEvents.ProcessPaymentSuccessfullyEvent)
			default:
